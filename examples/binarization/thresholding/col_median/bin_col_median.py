...
def get_col_median():
    col_median = []
    splitted_col_median = []
    with open(("col_median.txt"), 'r') as f:
        Lines = f.readlines()
        for line in Lines:
            col_median.append(line.rstrip())
    for i in range(0, len(col_median)):
        splitted_col_median.append(col_median[i].split(", "))
        splitted_col_median[i][0] = splitted_col_median[i][0].replace("[", "")
        splitted_col_median[i][-1] = splitted_col_median[i][-1].replace("]", "")
    return splitted_col_median

col_median = get_col_median()
def toBin(ind):
    binarizedResult = []
    for i in range(0, len(all_scd[ind])): # row
        res = []
        for j in range(0, len(all_scd[ind][i])):
            if all_scd[ind][i][j] < float(col_median[ind][j]):
                res.append(0)
            elif all_scd[ind][i][j] >= float(col_median[ind][j]):
                res.append(1)
