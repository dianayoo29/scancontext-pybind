...
query_cloud_filepath = os.path.join(data_dir, '003638.bin')
query_scd = bin2scd(query_cloud_filepath)
query_scd.tolist()

query_cloud_filepath_2 = os.path.join(data_dir, '000693.bin')
...
def toBin1(ind):
    binarizedResult = []
    for i in range(0, len(query_scd)): # egy sor az query_scd[ind][i]
        res = []
...
def toBin2(ind):
    binarizedResult = []
    for i in range(0, len(query_scd_2)): # egy sor az query_scd[ind][i]
        res = []
...
def scd_distance2(i_ind, j_ind):
    first_scd = toBin1(i_ind)
    sec_scd = toBin2(j_ind)
    hm_dist = hamming_distance2(first_scd, sec_scd)
    return hm_dist
...
print(scd_distance(3638, 693))
print(scd_distance2(3638, 693))
