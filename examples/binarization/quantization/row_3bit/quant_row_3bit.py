...
def get_quantiles():
    row_quant = []
    splitted_row_quant = []
    with open(("three_bit_quantiles_for_rows_" + args.sequence + ".txt"), 'r') as f:
        Lines = f.readlines()
        for line in Lines:
    ...
    for i in range(0, len(all_scd[ind])): # egy sor az all_scd[ind][i]
        ...
        for j in range(0, len(all_scd[ind][i])): # egy érték a soron belül
            ...
            if val < float(all_quantiles[ind][i][1]):
                ...
                res.append([0,1,0])
            elif val >= float(all_quantiles[ind][i][3]) and val < float(all_quantiles[ind][i][4]):
                res.append([1,0,0])
            elif val >= float(all_quantiles[ind][i][4]) and val < float(all_quantiles[ind][i][5]):
                res.append([0,1,1])
            elif val >= float(all_quantiles[ind][i][5]) and val < float(all_quantiles[ind][i][6]):
                res.append([1,0,1])
            elif val >= float(all_quantiles[ind][i][6]) and val < float(all_quantiles[ind][i][7]):
                res.append([1,1,0])
            elif val >= float(all_quantiles[ind][i][7]):
                res.append([1,1,1])
