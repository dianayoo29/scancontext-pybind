...
def toBin(ind):
    ...
    for row in all_val[ind]:
        ...
        distance = int(row[1])
        if height < 0:
            binarizedResult.append([0,0])
        elif height >= 0 and height < 2:
            binarizedResult.append([0,1])
        elif height >= 2 and height < 4:
            binarizedResult.append([1,0])
        elif height >= 4:
            binarizedResult.append([1,1])

        if distance < 5:
            binarizedResult[i] += [0,0]
        elif distance >= 5 and distance < 10:
            binarizedResult[i] += [0,1]
        elif distance >= 10 and distance < 15:
            binarizedResult[i] += [1,0]
        elif distance >= 15:
            binarizedResult[i] += [1,1]
