...
def toBin1(ind):
    ...
    for i in range(0, len(query_scd)): # row
        ...
        for j in range(0, len(query_scd[i])):
            if i == 0:
                if query_scd[i][j] <= query_scd[-1][j]:
                    res.append(0)
                elif query_scd[i][j] > query_scd[-1][j]:
                    res.append(1)
            else:
                if query_scd[i][j] <= query_scd[i-1][j]:
                    res.append(0)
                elif query_scd[i][j] > query_scd[i-1][j]:
                    res.append(1)
...
def toBin2(ind):
    ...
    for i in range(0, len(query_scd_2)): # row
        ...
        for j in range(0, len(query_scd_2[i])):
            if i == 0:
                if query_scd_2[i][j] <= query_scd_2[-1][j]:
                    res.append(0)
                elif query_scd_2[i][j] > query_scd_2[-1][j]:
                    res.append(1)
            else:
                if query_scd_2[i][j] <= query_scd_2[i-1][j]:
                    res.append(0)
                elif query_scd_2[i][j] > query_scd_2[i-1][j]:
                    res.append(1)
        binarizedResult.append(res)
    return binarizedResult
